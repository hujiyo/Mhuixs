...
- **Logex 原生操作语言**：内置脚本解释器，直接操作数据结构，无需 import
- **BigNum 统一数据类型**：任意精度数值、字符串、LIST、TABLE、BITMAP 的统一封装

**当前状态** (2026.01.19):
- ✅ **Logex 内置化完成**：13 个核心函数已内置，无需 import
...
Mhuixs 数据库核心
  ...
  │   └── STREAM（流数据）
  │
  ├── BigNum 统一类型（bhs/basic_handle_struct）
  │   ├── NUMBER（任意精度数值）
  │   ├── STRING（字符串）
  │   ├── LIST（列表指针）
  │   ├── TABLE（表指针）
...
1. **HOOK ≠ BigNum**
   ...
   - `BigNum`/`bhs` 只是 Logex 旧时代的数据类型名称
   - 两者是同一个结构体的不同别名：`typedef struct {...} BigNum, basic_handle_struct, bhs;`
...
Logex 是 Mhuixs 的原生操作语言，**无需 import** 即可使用核心功能：
...
### 脚本语言特性
- ✅ **内置核心函数**：LIST/TYPE/BITMAP 操作无需 import
- ✅ **任意精度计算**：BigNum 支持超大数值和高精度小数
...
```bash
cd src
gcc -o logex main.c evaluator.c lexer.c bignum.c builtin.c context.c \
...
```bash
# 编译测试程序
gcc -o test_builtin test_builtin_simple.c builtin.c bignum.c \
...
1. **HOOK 是核心，BigNum 是别名**
   - `HOOK` 注册系统是 Mhuixs 数据库的核心
   - `BigNum`/`bhs`/`basic_handle_struct` 是同一个结构体的不同名称
   - HOOK 体现"大海捞针"的数据库理念
...
4. **已完成 vs 规划中**
   - ✅ **已完成**: LIST/BITMAP/TYPE 内置函数、BigNum 统一类型、包管理系统
