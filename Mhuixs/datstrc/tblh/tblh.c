...
void gotoxy(uint32_t x, uint32_t y){/*原点为（0，0）*/printf("\033[%d;%dH", ++y, ++x);}
uint16_t sizeoftype(char type) {//返回某类型在p_data池中的占用的字节数	
	switch (type){
	case i1:return 1;
	case i2:return 2;
	case i4:return 4;
...
uint32_t ptfsizeoftype(char type){//根据数据类型返回打印时占用的字节数
	switch (type){
...
void cpstr(uint8_t* start, uint8_t* end, uint8_t* target){//复制start和end "中间" 的信息到target中
	for (int i = 1; i < (end - start); *target = *(start + i), target++, i++);
}
void flash_line_index(TABLE* table){//当p_data或record_length改变，更新line_index的快速指针
	uint8_t* new_p_data = table->p_data;
	uint32_t record_length = table->record_length, record_num = table->record_num-1;
	L_INDEX* line_index = table->line_index;
	for(;record_num>0;line_index[record_num].rcd_addr = new_p_data + record_length * record_num,record_num--);
	line_index[0].rcd_addr = new_p_data;
}
static void put_tobyte(void* void_inf, uint8_t* pbyte, uint8_t size){//把字节数为size的void_inf存放在pbyte指向的size个字节中
	if (size == 1) { //void_inf必须本身是对齐的(原生C变量)，这样不会字节未对齐
		...
		*(pbyte + 0) = inf;	*(pbyte + 1) = inf >> 8;
	}
	else if (size == 4) {
		uint32_t inf = *(uint32_t*)void_inf;
		*(pbyte + 0) = inf;	*(pbyte + 1) = inf >> 8;
		*(pbyte + 2) = inf >> 16; *(pbyte + 3) = inf >> 24;
	}
	else if (size == 8) {
		uint64_t inf = *(uint64_t*)void_inf;
		*(pbyte + 0) = inf;	*(pbyte + 1) = inf >> 8;
		*(pbyte + 2) = inf >> 16; *(pbyte + 3) = inf >> 24;
		*(pbyte + 4) = inf >> 32; *(pbyte + 5) = inf >> 40;
		*(pbyte + 6) = inf >> 48; *(pbyte + 7) = inf >> 56;
	}
}
static void get_frombyte(void* void_inf, const uint8_t* pbyte, uint8_t size){//读取i1,i2,i4,i8,ui1,ui2,ui4,ui8,f4,f8类型数据
	if (size == 1) {//注释请对照put_tobyte
		*(uint8_t*)void_inf = *pbyte;
	}
	else if (size == 2) {//tblh核心函数,无需考虑错误处理
		uint16_t inf = (*pbyte) | (*(pbyte + 1) << 8);
		*(uint16_t*)void_inf = inf;
	}
	else if (size == 4) {
		uint32_t inf = (*pbyte) | (*(pbyte + 1) << 8) |
			(*(pbyte + 2) << 16) | (*(pbyte + 3) << 24);
		*(uint32_t*)void_inf = inf;
	}
	else if (size == 8) {
		uint64_t inf = ((int64_t)*pbyte) | (((int64_t) * (pbyte + 1)) << 8) | (((int64_t) * (pbyte + 2)) << 16) | (((int64_t) * (pbyte + 3)) << 24) |
			(((int64_t) * (pbyte + 4)) << 32) | (((int64_t) * (pbyte + 5)) << 40) | (((int64_t) * (pbyte + 6)) << 48) | (((int64_t) * (pbyte + 7)) << 56);
...
static uint8_t store_fieldata(char* p_inputstr, uint8_t* p_storaddr, char type)
{
	...
	*/
	...
	case i2: {//标签后不能直接声明变量，用大括号括起来
		...
		put_tobyte(&i, p_storaddr, sizeoftype(i2));return 0;
	}
	case i4: {
		int32_t i = (int32_t)atoi((const char*)p_inputstr);
		put_tobyte(&i, p_storaddr, sizeoftype(i4));return 0;
	}		   
	case i8: {
	...
	case s50:
		for (int i = 0; i < short_string; i++) {
			if (*p_inputstr == separater || *p_inputstr == '\0')return 0;//当遇到separater和'\0'就跳出循环
	...
	case s300:
		for (int i = 0; i < long_string; i++) {
			if (*p_inputstr == separater || *p_inputstr == '\0')return 0;
			*p_storaddr++ = *p_inputstr++;//查了很久，这个写法不但不是未定义行为，而且似乎被大量使用
		}
		return 0;
	case d211:goto YEAR;
	case t111: goto HOUR;
	case dt1111: goto MONTH;
	case dt211111:goto YEAR;
	YEAR://年
	{
...
uint8_t tblh_make_table(TABLE* table, char* table_name, FIELD* field, uint32_t field_num)
{
	...
	table->p_field = (FIELD*)malloc(field_num * sizeof(FIELD));
	table->offsetofield = (uint32_t*)malloc(field_num * sizeof(uint32_t*));
	memset(table->p_field, 0, table->field_num * sizeof(FIELD));//归0
	uint32_t ofsum = 0;
	...
	uint8_t* p_data = (uint8_t*)malloc(table->record_length * initial_ROM);
	memset(p_data, 0, table->record_length * initial_ROM);//归0
	table->p_data = p_data;
	//创建line_index索引
	table->line_index = (L_INDEX*)malloc(sizeof(L_INDEX));//先建立一个line_index
	//创建table名区
	table->table_name = (char*)malloc(name_max_size);
	memset(table->table_name, 0, name_max_size);//归0
	memcpy(table->table_name, table_name, strlen(table_name)%name_max_size);
	return 0;
}
uint32_t tblh_add_record(TABLE* table, const char* record)//add总是在末尾追加
{
	/*
	tblh_add_record把新记录加到p_data的末尾、line_index的索引组织，未来可能还要处理分页管理
	函数返回 虚序列号
	*/
	if (table->record_num == table->data_ROM) {	//记录数容量满了:扩展//不用担心有空位没有利用，记录条数永远等于实际记录条数
	...
	//定位本条待写入记录的首地址即p_data的末尾    （不用担心内存中存在空位 ：tblh_rmv_record将会把最后一个数据填补到删除的记录处）
	...
	for (uint32_t i = 0; i < field_num; i++)
	{
		...
		if (i != field_num - 1)cpstr((uint8_t*)(pointer - 1), (uint8_t*)strchr(pointer, separater), (uint8_t*)result);//获得本字段对应的字符串
		else cpstr((uint8_t*)(pointer - 1), (uint8_t*)strchr(pointer, '\0'), (uint8_t*)result);
		store_fieldata(result, new_record_address + table->offsetofield[i], table->p_field[i].type);//store_fieldata自动将目标字符串按照type的种类存入p_data中
	}
	return table->record_num - 1;
}
uint8_t tblh_rmv_record(TABLE* table, uint32_t j)//删除虚序号及其对应的record
{
	/*
	...
	*/
	//判断j记录是否存在
	if (j >= table->record_num)return 1;
	...
	if (table->line_index[j].sequence != table->record_num - 1) {
		...
		for (; j_mo < table->record_num; j_mo++) {
			if (table->line_index[j_mo].sequence == table->record_num - 1)break;//找到末记录对应的虚序列
		}
		//把末记录复制到j处
		cpstr(table->line_index[j_mo].rcd_addr - 1, table->line_index[j_mo].rcd_addr + table->record_length, table->line_index[j].rcd_addr);
...
uint8_t tblh_swap_record(TABLE* table, uint32_t j1, uint32_t j2)//函数用于交换两条记录的排序 返回0表示成功
{
	//tblh的实现方法是虚序列号对应的索引的交换
	if (j1 >= table->record_num || j2 >= table->record_num)return 1;//验证j1、j2是否存（j1，j2一定是正数）
...
uint8_t tblh_rmv_field(TABLE* table, uint32_t i)
{
	...
	*/
	...
	if (i != table->field_num - 1) {
		...
		for (uint32_t ii = i; ii < table->field_num - 1; table->p_field[ii] = table->p_field[ii + 1], table->offsetofield[ii] = table->offsetofield[ii + 1], ii++);//索引进位
	...
	*/
	...
	if (new_offset == table->record_usage) {
		...
		if (table->record_length - table->record_usage <= new_field_size) {
			...
			//接下来开始进行所有数据的位移
			if (table->record_num == 0)goto l;//uint32_t-1的情况
		...
		l:flash_line_index(table);//刷新line_index，至此，扩容完毕		
		}
...
{
	if (i1_ >= table->field_num || i2_ >= table->field_num)return 1;//验证11、12是否存
...
uint8_t tblh_insert_field(TABLE* table, FIELD* field, uint32_t i)
{
	...
	if (i >= sep)return 0;//如果非法插入还是默认在最后插入
...
uint8_t tblh_getfrom_i_j(TABLE* table, uint32_t i, uint32_t j, void* buffer)
{
	...
	*/
	...
	switch (type)
	{
		...
		case i1:*(int8_t*)buffer = *(int8_t*)inf_addr; return 0;
		case i2:get_frombyte(buffer, inf_addr, 2); return 0;
		case i4:get_frombyte(buffer, inf_addr, 4); return 0;
		case i8:get_frombyte(buffer, inf_addr, 8); return 0;
		case ui1:*(uint8_t*)buffer = *(uint8_t*)inf_addr; return 0;
		case ui2:get_frombyte(buffer, inf_addr, 2); return 0;
		case ui4:get_frombyte(buffer, inf_addr, 4); return 0;
		case ui8:get_frombyte(buffer, inf_addr, 8); return 0;
		case f4:get_frombyte(buffer, inf_addr, 4); return 0;
		case f8:get_frombyte(buffer, inf_addr, 8); return 0;
			//s50,s300本质上就是字符串
		case s50:memcpy(buffer, inf_addr, 50); return 0;
		case s300:memcpy(buffer, inf_addr, 300); return 0;
		/*
...
void tblh_printf_record(TABLE* table, uint32_t j, uint32_t y)
{
	...
	for (uint32_t i = 0; i < table->field_num; i++) {
		ptfmap[i] = address;
		address += max(ptfsizeoftype(table->p_field[i].type), strlen(table->p_field[i].name) + 1);
	}
	for (uint32_t i = 0; i < table->field_num; i++) {
			...
			switch (table->p_field[i].type) {
				case i1:printf("%d", *(int8_t*)inf_addr); break;
				case i2: {
					int16_t tp; get_frombyte(&tp, inf_addr, sizeoftype(i2));
					printf("%d", tp); break;
				}
				case i4: {
					int32_t tp; get_frombyte(&tp, inf_addr, sizeoftype(i4));
					printf("%d", tp); break;
				}
				case i8: {
					int64_t tp; get_frombyte(&tp, inf_addr, sizeoftype(i8));
					printf("%lld" /*PRIx64*/, tp); break;//"lld"报错，不知道为什么
				}
				case ui1:printf("%d", *(uint8_t*)inf_addr); break;
				case ui2: {
					uint16_t tp; get_frombyte(&tp, inf_addr, sizeoftype(ui2));
					printf("%d", tp); break;
				}
				case ui4: {
					uint32_t tp; get_frombyte(&tp, inf_addr, sizeoftype(ui4));
					printf("%d", tp); break;
				}
				case ui8: {
					uint64_t tp; get_frombyte(&tp, inf_addr, sizeoftype(ui8));
					printf("%llu"/* PRIu64*/, tp); break;
				}
				case f4: {
					float tp; get_frombyte(&tp, inf_addr, sizeoftype(f4));
					printf("%.7g", tp); break;
				}
				case f8: {
					double tp; get_frombyte(&tp, inf_addr, sizeoftype(f8));
					printf("%.15g", tp); break;
				}
				case s50:printf("%.10s", inf_addr); break;
				case s300:printf("%.30s", inf_addr); break;
				case d211: {
					tp_d211 tp; tblh_getfrom_i_j(table, i, j, &tp);
					printf("%d.%d.%d", tp.year, tp.month, tp.day); break;
				}
				case t111: {
					tp_t111 tp; tblh_getfrom_i_j(table, i, j, &tp);
					printf("%d:%d:%d", tp.hour, tp.minute, tp.second); break;
				}
				case dt1111: {
					tp_dt1111 tp; tblh_getfrom_i_j(table, i, j, &tp);
					printf("%d.%d.%d:%d", tp.month, tp.day, tp.hour, tp.minute); break;
				}
				case dt211111: {
					tp_dt211111 tp; tblh_getfrom_i_j(table, i, j, &tp);
					printf("%d.%d.%d.%d:%d:%d", tp.year, tp.month, tp.day, tp.hour, tp.minute, tp.second); break;
				}
			}
		}
	free(ptfmap);
...
void tblh_del_table(TABLE* table){//删除表，此时表必须make或者load之后才能使用
	table->field_num=0;	
	table->map_size=0;	
	...
	table->record_num=0;	
	table->data_ROM=0;
	if(table->p_field!=NULL)free(table->p_field);
	if(table->p_data!=NULL)free(table->p_data);	
	if(table->line_index!=NULL)free(table->line_index);
	if(table->offsetofield!=NULL)free(table->offsetofield);
	if(table->idle_map!=NULL)free(table->idle_map);
	if(table->table_name!=NULL)free(table->table_name);
}
...
uint8_t tblh_load_table(TABLE* table,char* file_path){//加载表，删除原表内容
	//先释放原有空间
	...
	//申请空间
	table->p_field = (FIELD*)calloc(1,sizeof(FIELD)*head.field_num);
	table->offsetofield = (uint32_t*)calloc(1,sizeof(uint32_t) * head.field_num);
	table->idle_map = (IDLE_MAP*)calloc(1,sizeof(IDLE_MAP) * head.map_size);
	table->line_index = (LINE_INDEX*)calloc(1,sizeof(LINE_INDEX) * head.record_num);
	table->p_data = (uint8_t*)calloc(1,sizeof(uint8_t) * head.data_ROM);
	table->table_name = (char*)calloc(1,sizeof(char) * name_max_size);
	//读取表名
	fseek(file,head.offset_of_table_name,SEEK_SET);
