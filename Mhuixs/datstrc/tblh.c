...
static void cpstr(uint8_t* start, uint8_t* end, uint8_t* target){//复制start和end "中间" 的信息到target中
	for (int i = 1; i < (end - start); *target = *(start + i), target++, i++);
}
static void flash_line_index(TABLE* table){//当p_data或record_length改变，更新line_index的快速指针
...
static void get_frombyte(void* void_inf, const uint8_t* pbyte, uint8_t size){//读取i1,i2,i4,i8,ui1,ui2,ui4,ui8,f4,f8类型数据
	if (size == 1) {//注释请对照put_tobyte
		...
		*(uint32_t*)void_inf = inf;
		...
		uint64_t inf = ((int64_t)*pbyte) | (((int64_t) * (pbyte + 1)) << 8) | 
		(((int64_t) * (pbyte + 2)) << 16) | (((int64_t) * (pbyte + 3)) << 24) |
		(((int64_t) * (pbyte + 4)) << 32) | (((int64_t) * (pbyte + 5)) << 40) | 
		(((int64_t) * (pbyte + 6)) << 48) | (((int64_t) * (pbyte + 7)) << 56);
...
uint32_t tblh_add_record(TABLE* table, const char* record)//add总是在末尾追加
{
	...
	*/
	...
	if (isnullrecord)return table->record_num - 1;//如果是空记录，则不写入数据
