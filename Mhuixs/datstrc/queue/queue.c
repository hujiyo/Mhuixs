...
int putintoQUEUE(QUEUE* queue,uint8_t* stream,uint32_t len){
    if(queue == NULL || len == 0 || stream == NULL)return err;
    //申请节点
    NODE_Q* new_node = (NODE_Q*)calloc(sizeof(NODE_Q),1);
    ...
    new_node->len = len;
    new_node->next = NULL;
    memcpy(new_node->data,stream,len);//至此，新节点已经初始化完毕
    //将节点插入队列末尾
...
int getfromQUEUE(QUEUE* queue,uint8_t* stream,uint32_t len){
    //你必须保证stream的大小足够容纳队列头元素
    if(queue == NULL||queue->node_num == 0)return err;
