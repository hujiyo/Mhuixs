...
uint8_t* TABLE::real_addr_of_lindex(uint32_t j)//index是虚序列
{
    ...
    */
	return this->p_data + this->line_index.get_index(j) * this->record_length;
}
...
uint8_t* TABLE::address_of_i_j(uint32_t i,uint32_t j)
{
    ...
    等价于：return real_addr_of_lindex(j) + this->field_offset[i];
    */
	return this->p_data + this->line_index.get_index(j) * this->record_length + this->field_offset.get_index(i);
}
