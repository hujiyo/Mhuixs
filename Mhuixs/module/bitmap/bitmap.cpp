...
// 位拷贝函数：从源地址指定位置开始，拷贝指定位数到目标地址指定位置  仅支持小端序
...
void bitcpy(uint8_t* destination, uint8_t dest_first_bit, const uint8_t* source, uint8_t source_first_bit, uint32_t len) {
    ...
    while (len > 0) {
        uint8_t bits_to_copy = MIN(MIN(8 - s_bit, 8 - d_bit), len);

        // 跨字节读取
        uint16_t src_word = source[s_byte];
        if (s_bit + bits_to_copy > 8) {
            src_word |= (source[s_byte + 1] << 8);
        }
        uint8_t src_bits = (src_word >> s_bit) & ((1 << bits_to_copy) - 1);
...
BITMAP::BITMAP():bitmap((uint8_t*)calloc(4,1)){
    if(this->bitmap == NULL){
        #ifdef bitmap_debug
...
BITMAP::BITMAP(const char* s){
    ...
    this->bitmap = (uint8_t*)malloc(4+len/8+1);
    if (!this->bitmap) {
        #ifdef bitmap_debug
        perror("BITMAP memmery init failed!");
        #endif
...
BITMAP::BITMAP(char* s,uint32_t len,uint8_t zerochar){
...
inline BITMAP::BIT BITMAP::operator[](uint32_t offset){
...
int main() {
    ...
    // 测试count
    uint32_t cnt = bm1.count(0, bm1.size()-1);
    printf("bm1 1的个数: %u\n", cnt);

    // 测试find
    int64_t pos = bm1.find(1, 0, bm1.size()-1);
    printf("bm1 第一个1的位置: %lld\n", pos);

    // 测试set(char*,len,zerochar)
    char teststr[] = "01010101";
    bm2.set(0, 8, teststr, '0');
    printf("bm2 set(0,8,\"01010101\",'0'): ");
    bm2.ptf();

    // 测试rexpand
    bm3.set(15, 1, 1); // 自动扩容
    printf("bm3 set(15,1,1)后: ");
    bm3.ptf();
...
bm2(拷贝自bm1): 10110011
bm3(赋值自bm1): 10110011
bm1 += bm4: 101100111111
bm1 set(2,3,0): 100000111111
bm1 [1]=1, [2]=1: 111000111111
bm1 1的个数: 9
bm1 第一个1的位置: 0
bm2 set(0,8,"01010101",'0'): 01010101
bm3 set(15,1,1)后: 1011001100000001
bm3 [0]=0, [15]=0: 0011001100000000
所有测试完成。
